...
{	
	...
	zombie_utility::set_zombie_var( "zombie_powerup_fire_sale_time",		N_POWERUP_DEFAULT_TIME );
	zombie_utility::set_zombie_var( "zombie_powerup_bonfire_sale_on",		false );
	zombie_utility::set_zombie_var( "zombie_powerup_bonfire_sale_time",		N_POWERUP_DEFAULT_TIME );
	zombie_utility::set_zombie_var( "zombie_powerup_insta_kill_on",			false, undefined, undefined, true );
	zombie_utility::set_zombie_var( "zombie_powerup_insta_kill_time",		N_POWERUP_DEFAULT_TIME, undefined, undefined, true );	// length of insta kill
	zombie_utility::set_zombie_var( "zombie_powerup_double_points_on",		false, undefined, undefined, true );
	zombie_utility::set_zombie_var( "zombie_powerup_double_points_time",	N_POWERUP_DEFAULT_TIME, undefined, undefined, true );	// length of point doubler
...
function init_powerups()
{
	...
	add_zombie_powerup( "insta_kill_ug",		"zombie_skull",					&"ZOMBIE_POWERUP_INSTA_KILL",	&func_should_never_drop, POWERUP_ONLY_AFFECTS_GRABBER, !POWERUP_ANY_TEAM, !POWERUP_ZOMBIE_GRABBABLE, undefined, CLIENTFIELD_POWERUP_INSTANT_KILL_UG, "zombie_powerup_insta_kill_ug_time", "zombie_powerup_insta_kill_ug_on", VERSION_SHIP );
	...
	clientfield::register( "scriptmover", CLIENTFIELD_POWERUP_FX_NAME, VERSION_SHIP, 3, "int" );
}
...
function powerup_hud_monitor( )
{
	...
	flashing_timers = [];
	flashing_values = [];
	flashing_timer = POWERUP_FLASHING_STAGE_ONE_TIME;
	flashing_delta_time = 0;
	flashing_is_on = false;
	flashing_value = CLIENTFIELD_POWERUP_STATE_FLASHING_OFF;
	flashing_min_timer = POWERUP_FLASHING_STAGE_TWO_DELTA_TIME+0.05;
	while( flashing_timer >= flashing_min_timer )
	{
		if ( flashing_timer < POWERUP_FLASHING_STAGE_TWO_TIME)
		{
			flashing_delta_time = POWERUP_FLASHING_STAGE_TWO_DELTA_TIME;
		}
		else
		{
			flashing_delta_time = POWERUP_FLASHING_STAGE_ONE_DELTA_TIME;
		}

		if( flashing_is_on )
		{
			flashing_timer = flashing_timer - flashing_delta_time - 0.05;
			flashing_value = CLIENTFIELD_POWERUP_STATE_FLASHING_ON;
		}
		else
		{
	...
	while ( true )
	{
		...
		for( playerIndex = 0; playerIndex < players.size; playerIndex++ )
		{
			for ( client_field_key_index = 0; client_field_key_index < client_field_keys.size; client_field_key_index++ )
			{
				...
				{
					player clientfield::set_to_player( client_field_name, CLIENTFIELD_POWERUP_STATE_OFF );
				}
			}
		}
	}
...
function set_clientfield_powerups( clientfield_name, powerup_timer, powerup_on, flashing_timers, flashing_values )
{
	if ( powerup_on )
	{
		if ( powerup_timer < POWERUP_FLASHING_STAGE_ONE_TIME )
		{
			// flashing stage
			flashing_value = CLIENTFIELD_POWERUP_STATE_FLASHING_OFF;
		...
		{
			self clientfield::set_to_player( clientfield_name, CLIENTFIELD_POWERUP_STATE_ON );
		}
	}
	else
	{
		self clientfield::set_to_player( clientfield_name, CLIENTFIELD_POWERUP_STATE_OFF );
	}
}
...
function minigun_no_drop()
{
	...
	if( !level flag::get( "power_on" ) ) // if power is not on check for solo
	{
		...
		{
			if( !isdefined( level.solo_lives_given ) || level.solo_lives_given == 0 ) // the power isn't on, it is a solo game, has the player purchased a life/revive?
			{
...
{
	DEFAULT( level.zombie_statless_powerups, [] );

	level.zombie_statless_powerups[ powerup_name ] = true;
}
...
function powerup_zombie_grab(powerup_team)
{
	...
	zombie_grab_trigger = spawn( "trigger_radius", self.origin - (0,0,40), SPAWNFLAG_TRIGGER_AI_AXIS + SPAWNFLAG_TRIGGER_NOT_PLAYER, 32, 72 );
...
function powerup_grab(powerup_team)
{
	...
	while (isdefined(self))
	{
		...
		for (i = 0; i < grabbers.size; i++)
		{
			...
			if( IsDefined( player.is_drinking ) && IS_DRINKING( player.is_drinking ) && IsDefined( level._custom_powerups ) && IsDefined( level._custom_powerups[ self.powerup_name ] ) && IS_TRUE( level._custom_powerups[ self.powerup_name ].prevent_pick_up_if_drinking ) )
			{
				continue;
			}
			...
			if ( DistanceSquared( grabber.origin, self.origin ) < range_squared || ignore_range )
			{
				...
				*/
				...
				if( should_award_stat ( self.powerup_name ) && IsPlayer(player) ) //don't do this for things that aren't really a powerup
				{
...
function check_for_instakill( player, mod, hit_location )
{
	...
	if( IsDefined( player ) && IsAlive( player ) && ( level.zombie_vars[player.team]["zombie_insta_kill"] || IS_TRUE( player.personal_instakill )) )
	{
		...
		modName = zm_utility::remove_mod_from_methodofdeath( mod );
		if( !level flag::get( "special_round" ) && !IS_TRUE( self.no_gib ) ) // normal zombie round, and zombie is gibbable
		{
			self zombie_utility::zombie_head_gib();
		}
		self.health = 1;
...
function point_doubler_on_hud( drop_item, player_team )
{
	...
	if ( level.zombie_vars[player_team]["zombie_powerup_double_points_on"] )
	{
		...
		level.zombie_vars[player_team]["zombie_powerup_double_points_time"] = N_POWERUP_DEFAULT_TIME;
		return;
	}
