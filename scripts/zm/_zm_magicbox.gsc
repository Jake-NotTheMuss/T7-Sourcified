...
function treasure_chest_think()
{
	...
	else
	{
		if( !IS_TRUE( self.unbearable_respin ) )
		{
			...
			self.chest_user = user;
			bb::logpurchaseevent( user, self, user_cost, self.grab_weapon, false, "_magicbox", "_offered" );
			weaponIdx = undefined;
		...
		while( !IS_TRUE(self.closed_by_emp) )
		{
			...
			if( grabber == user || grabber == level )			
			{
				...
				{
					bb::logpurchaseevent( user, self, user_cost, self.zbarrier.weapon, false, "_magicbox", "_grabbed" );
